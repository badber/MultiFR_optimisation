...
% Author: Luis Badesa
    ...
    clear i j
    Bounds = [Bounds,...
              0 <= DV_Total_FR_atTd <= sum(FR_capacity)*ones(1,length(Td))];
    ...
    % Now define bounds:
    UpperBound_FR_term = sum(FR_capacity./Td);
    ...
    UpperBound_Inertia_term = (H_const'*Pmax)/InputData.f_0+...
                    (H_const_Wind*Wind)/InputData.f_0 +...
                    (H_const_Nuclear*PLossMax)/InputData.f_0;
    UpperBound_Inertia_term = UpperBound_Inertia_term*ones(1,length(Td)); 
    LowerBound_Inertia_term = - sum(FR_capacity.*Td/(4*nadir_req));
    LowerBound_Inertia_term = LowerBound_Inertia_term*ones(1,length(Td)); 
    LowerBound_PLoss_term = (PLossMax-sum(FR_capacity));
